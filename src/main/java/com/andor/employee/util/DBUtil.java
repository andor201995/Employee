...
public class DBUtil {
	...
	public static DBCollection getDBCollection(String collName) throws DAOException {
		if( client == null
	...
	private static void openDBConnection() throws DAOException {
		try {
			...
			final String DB_HOST = dbProperties
					.getProperty("HOST_NAME", "localhost");
			final int DB_PORT = Integer.parseInt(dbProperties
					.getProperty("DB_PORT", "5000"));
			...
			final MongoCredential credential = MongoCredential
					.createScramSha1Credential(DB_ID, DB_NAME, DB_PASSWORD
							.toCharArray());
			client = new MongoClient(new ServerAddress(DB_HOST, DB_PORT), Arrays
					.asList(credential));
		} catch (MongoCommandException | FileNotFoundException
				| NumberFormatException | NullPointerException e) {
			LOGGER.error("error is connecting to DB", e);
